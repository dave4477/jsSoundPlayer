export default class BandControls {
	...
	createControl() {
		...
		let bandFilters = this.sound.getNodeByName("bandFilters");
		bandFilters = bandFilters && bandFilters.value.bandFilters ? bandFilters.value.bandFilters : [];
	...
	createFilterSlider(id, arrRange, index) {
		...
		this.sliders[id].onchange = (e) => {
			this.sound.getNodeByName("bandFilters").value.bandFilters[index].gain.setValueAtTime(e.target.value, 0);
		}
